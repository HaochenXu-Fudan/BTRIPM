...
Tz =opts.Tz;    s1 = opts.s1;%%% to speed up in cal z
difffy = opts.difffy;
m = opts.m;
n = opts.n;
%%%
...
Delta_bar = 10*sqrt(length(x));
Delta0 = Delta_bar/8;
if isfield(opts,'Delta')
    Delta = opts.Delta;
    Delta0 = Delta;% Add by chen
    Delta_bar = 8*Delta;
else
    Delta = Delta0;
end
...
%%%
% 将 |tCG.m| 存放的目录加入工作目录。
% addpath('\R2020b\research\Bi_op\Interior_method\newton');
...
update = 1;% label whether x was updated in the last iter.% by ch on 2022/1/21
%% 迭代主循环
% 以 |maxit| 为最大迭代次数。
for iter = 1:maxit
    %%%
...
%     hess_tCG = @(d) hess(x,d);
    if update% by ch on 2022/1/21
        z = x(m+1:m+n); % notice x_thisfile = [x_former;y_former];
        x_former = x(1:m);
        for i=1:Tz
            z = z-difffy(x_former,z)*s1;
        end
        hess_tCG = @(d) hess(x,z,d);
    end
%     above is code modified, which avoid hundreds of repeated computation
%     of z in hess_tCG.
    ...
    update = 0;% by ch on 2022/1/21
    if ratio >= eta1
        x = xnew;   fp = f;  gp = g;
        out.nrmG = [out.nrmG; nrmg];
        update = 1;% by ch on 2022/1/21
    end
    %%%
    % 计算函数值相对变化。
